...
<div class="w-full border-opacity-50 py-8 my-8 overflow-hidden">
    ...
    <div class="flex flex-row flex-nowrap justify-between max-w-screen !overflow-hidden">
        {#each clients_grouped as clients_group, i}
            <div class="grid h-32 fill-gray-500">
                {#if i === 0}
                <div class="!overflow-hidden bg-clip-content">
                    {#key j}
                    <img in:fly="{{ y: -50, duration: 300, delay: 500 }}" out:fly="{{ y: 50, duration: 100 }}" class="p-3 sm:p-6 object-contain aspect-auto h-32 w-32" src={clients_group[j].logo.url} alt={clients_group[j].name}>
                    {/key}
                </div>
                {:else if i === 1}
                    <div class="!overflow-hidden">
                        {#key k}
                        <img in:fly="{{ y: -50, duration: 300, delay: 500 }}" out:fly="{{ y: 50, duration: 100 }}" class="p-3 sm:p-6 object-contain aspect-auto h-32 w-32" src={clients_group[k].logo.url} alt={clients_group[k].name}>
                        {/key}
                    </div>
                {:else if i === 2}
                    <div class="!overflow-hidden">
                        {#key l}
                        <img in:fly="{{ y: -50, duration: 300, delay: 500 }}" out:fly="{{ y: 50, duration: 100 }}" class="p-3 sm:p-6 object-contain aspect-auto h-32 w-32" src={clients_group[l].logo.url} alt={clients_group[l].name}>
                        {/key}
                    </div>
                {/if}
            </div>
